...
float NeuralNetwork::findMinAV(Neuron* neuron, NeuralNetwork& loadState) {
...
void NeuralNetwork::getSamplePoints(Neuron* neuron, std::vector<float>* centroid,
	std::vector<float>* samplePoints, std::ifstream& file) {
		...
		for (int s = 0; s < sampleSize; s++) {
			...
			float minCostAV = findMinAV(neuron, loadState);
...
void NeuralNetwork::trainNeuralNetwork(std::string fileName,
	...
	while (!file.eof()) {
		...
		// Contains all the points containing the inputs (child nodes)
		...
		getSamplePoints(neuron, &centroid, &samplePoints, file);
		...
		for (int s = 0; s < sampleSize; s++) {
			...
			STOP DOES NOT FOLLOW FORMULAR (fetch from PC)
