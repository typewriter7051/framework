...
float DynamicNeuron::getValue(std::vector<float>* profile, std::vector<float>* oldProfile, std::vector<bool>* completionRecord) {
	...
	for (DynamicNeuron::DynamicNeuralConnection nc : ncs) {
		...
		if (!isComplete)
			nc.prevNeuron->getValue(profile, oldProfile, completionRecord);

		// I leave this as an if else statement for compiler optimization.
		// Otherwise go branchless.
		if (nc.isRecursive)
			temp += oldProfile->at(prevID) * nc.weight;
		else
...
void DynamicNeuron::addConnection(DynamicNeuron* n, bool isRecursive) {
	ncs.push_back(DynamicNeuralConnection(n, isRecursive));
}
...
float DynamicNeuron::getAVDerivative(std::vector<float>* profile) {
	...
	// TODO: fix and optimize these fucking AFs they done broke.
	switch (af) {
	case arcHyperTan:
		return 1 / (1 - av * av);
	case ELU:
		return (av < 0) ? exp(av) : 1;
	case hyperTan:
		a = tanh(av);
		return 1 - a * a;
	case identity:
